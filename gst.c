...
/* Insert functions */
void gst_add_char(struct gst *gst, uint32_t ch) /* insert character, increase sa[sid] if needed */
{
  int sid = gst->sid;
...
void gst_line_end(struct gst *gst)	/* insert end character, create empty string */
{
...
void gst_get_longest_strings(struct gst *gst, int *count, uint32_t ***strings)
{
  ...
  gst_get_strings(gst, &tmp_count, &tmp_strings);
  int max_len =  0;
  int count_max = 0;
  int i;
